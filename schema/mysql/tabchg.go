...
func processColumn(db common.DB, tabName string, oldCol, newCol *schema.Column) error {
	...
	if oldCol.Name != newCol.Name {
		strSQL = fmt.Sprintf("alter table %s CHANGE column %s %s", tabName, oldCol.Name, dbDefine(newCol))
		if _, err := db.Exec(strSQL); err != nil {
			err = common.NewSQLError(err, strSQL)
			log.Println(err)
			return err
	...
	if !oldCol.Eque(newCol) {
		strSQL = fmt.Sprintf("alter table %s MODIFY %s", tabName, dbDefine(newCol))
		if _, err := db.Exec(strSQL); err != nil {
			err = common.NewSQLError(err, strSQL)
			log.Println(err)
			return err
