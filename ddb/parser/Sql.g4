...
selectStatement:
	SELECT selectElements FROM tableSources joinClause (
		whereClause
...
expr:
	...
	| '(' selectStatement ')'
	| '(' expr ')'
	| CASE WHEN logicExpression THEN expr (
		WHEN logicExpression THEN expr
	)* (ELSE expr)? END
...
value: decimalLiteral | textLiteral | bind_variables;

functionCall: aggregateFunction | commonFunction;

aggregateFunction: (AVG | MAX | MIN | SUM) '(' functionArg ')'
...
orderByExpression:
	expr order = (ASC | DESC)? ((NULLS FIRST) | (NULLS LAST))?;
...
//MySQL, PostgreSQL Oracle supports rownum
